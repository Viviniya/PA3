...
int reset_locks(int pid){
    ...
    int i ;
    for(i=0; i < NLOCKS;i++){
      locks_table[i].process_locked[pid]= -1;
    }
    restore(ps);
    return OK;
}

int release_held_locks(int pid){
    STATWORD ps;
    disable(ps);
    int i ;
    for(i=0;i < NLOCKS;i++){
          if(locks_table[i].process_locked[pid] == LOCKED){
            releaseall(1, i);
        }
    }
	//	reset_locks( pid);
